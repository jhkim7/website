...
class fortune_latest_news extends WP_Widget {

// constructor
    function fortune_latest_news() {
		$widget_ops = array('classname' => 'moesia_latest_news_widget', 'description' => __( 'Show the latest news from your blog.', 'moesia') );
        parent::WP_Widget(false, $name = __('Moesia FP: Latest News custom', 'moesia'), $widget_ops);
...
?>
		<section id="latest-news" class="latest-news-area">
			<div class="container">
				...
				<div class="clearfix">
					<?php while ( $r->have_posts() ) : $r->the_post(); ?>
						<div class="blog-post col-12">											
							<?php the_title( sprintf( '<h4 class="entry-title-custom"><a href="%s" rel="bookmark">', esc_url( get_permalink() ) ), '</a></h4>' ); ?>
							<?php 
				...
				<a href="<?php echo esc_url(get_category_link(get_cat_ID($cat -> name))); ?>" class="all-news-cusom">
				<?php else : ?>
				<a href="<?php echo get_permalink( get_option( 'page_for_posts' ) ); ?>" class="all-news-custom">
				<?php endif; ?>
					<?php if (!$see_all_text) : ?>
						<?php echo __( 'more blog', 'moesia' ); ?>					
					<?php else : ?>
